...
- Товары с ручной выдачей создаются в Digiseller как `Arbitrary` с содержимым `Form`; `UniqueFixed` не поддерживает форму.
- Для Plati.Market товар привязывается к точной категории игры из авторизованного каталога Digiseller через `owner: 1`; ID старого дерева площадки не подходят.
