...
export const productsTable = pgTable(
  "sync_products",
  {
    ...
    gpayId: integer("gpay_id").notNull(),
    digisellerId: integer("digiseller_id").unique(),
    previousDigisellerId: integer("previous_digiseller_id"),
    digisellerDeliveryType:
      text("digiseller_delivery_type").$type<DigisellerDeliveryType>(),
  ...
  (table) => [
    uniqueIndex("sync_products_gpay_id_product_type_unique").on(
      table.gpayId,
      table.productType,
    ),
  ],
