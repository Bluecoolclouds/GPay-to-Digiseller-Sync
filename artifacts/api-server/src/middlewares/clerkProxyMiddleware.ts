...
}, allowedHosts?: ReadonlySet<string>): string | undefined {
  const forwarded = req.headers['x-forwarded-host'];
  const raw = Array.isArray(forwarded) ? forwarded[0] : forwarded;
  const firstHop = raw?.split(',')[0]?.trim();
  const host = firstHop || req.headers.host?.trim() || undefined;
  if (!host) return undefined;
  if (process.env.NODE_ENV !== "production" && /^localhost(?::\d+)?$/.test(host)) {
    return host;
  }
  return !allowedHosts || allowedHosts.has(host) ? host : undefined;
}
  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        if (!host) {
          proxyReq.destroy(new Error('Untrusted Clerk proxy host'));
          return;
        }
