  ...
  "../../node_modules/.pnpm/pino@9.14.0/node_modules/pino/pino.js"(exports, module) {
    function pinoBundlerAbsolutePath(p) {
      try {
        ...
        const outputDir = "/home/runner/workspace/artifacts/api-server/dist-tests";
...
// src/__tests__/digiseller-price-tasks.test.ts
import assert from "node:assert/strict";
import { afterEach, test } from "node:test";

// src/lib/digiseller.ts
import { createHash } from "node:crypto";
import { execFile } from "node:child_process";
import { promisify } from "node:util";

// src/lib/ai-product-image.ts
...
// src/lib/digiseller.ts
function getDigisellerError(json, fallback) {
  ...
  const description = typeof json.retdesc === "string" ? json.retdesc : json.retdesc ? JSON.stringify(json.retdesc) : "";
  return details || description || fallback;
}
async function loginDigiseller() {
...
async function updateDigisellerProductPrices(prices, providedToken, polling = {}) {
  ...
  let taskId2;
  try {
    task = JSON.parse(taskBody);
    taskId2 = task.taskId ?? task.TaskId;
  } catch {
  ...
  const timeoutMs = polling.timeoutMs ?? 6e4;
  const pollIntervalMs = polling.pollIntervalMs ?? 1e3;
  ...
  while (now() < deadline) {
    ...
    statusUrl.searchParams.set("taskId", taskId2);
  ...
  throw new Error(
    `Digiseller \u043D\u0435 \u0437\u0430\u0432\u0435\u0440\u0448\u0438\u043B \u043E\u0431\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u0435 \u0446\u0435\u043D \u0437\u0430 ${Math.ceil(timeoutMs / 1e3)} \u0441\u0435\u043A\u0443\u043D\u0434`
  );
}
...
var taskId = "12345678-1234-1234-1234-123456789abc";
afterEach(() => {
  globalThis.fetch = originalFetch;
});
function mockPriceTask(taskResponse, statuses) {
  const requestedStatuses = [];
  globalThis.fetch = async (input) => {
    const url = String(input);
    if (url.includes("/api/product/edit/prices")) {
      return typeof taskResponse === "string" ? new Response(taskResponse) : Response.json(taskResponse);
    }
...
//# sourceMappingURL=digiseller-price-tasks.test.mjs.map
